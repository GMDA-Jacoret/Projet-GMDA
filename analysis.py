...
# Compute and add c1
success_rate = raw[['dd', 'jit_coef', 'halving_depth']].groupby(['dd', 'jit_coef'], as_index=False).count()/10
...
for k, res in enumerate([res_t1, res_t2]):
    ...
    hd = hd_df.plot(marker = '.',
            title = 'Type %i : Evolution of the "halving depth" with the doubling dimension' %(k+1),)
    ...
    # plot
    c0 = c0_df.plot(marker = '.', title = 'Type %i : Evolution of the c_0 with the doubling dimension' %(k+1))
    ...
    # plot
    c1 = c1_df.plot(marker = 'o', title = 'Type %i : Evolution of the c_0 with the doubling dimension' %(k+1))
    ...
    # plot
    c2 = c2_df.plot(marker = '.', title = 'Type %i : Evolution of the c_0 with the doubling dimension' %(k+1))
